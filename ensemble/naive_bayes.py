...
# function to train and evaluate model with different parameters
def train_and_evaluate_model(X_train, Y_train, X_test, Y_test, alpha=1.0, fit_prior=True):
...
# function to train and evaluate model with different parameters
def train_and_evaluate_model(X_train, Y_train, X_test, Y_test, alpha, fit_prior):
    print("Training Naive Bayes Model...")
    start_time = time.time()
    nb_model = MultinomialNB(alpha=alpha, fit_prior=fit_prior)
    nb_model.fit(X_train, Y_train)
